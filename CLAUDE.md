...
- **Characters** (`config.js` `MK.CHARACTERS`): ★1–5 stats → physics via `MK.deriveStats` → `{maxSpeed, accel, turnRate, weight}`. Models are built procedurally in `characters.js` (`MK.Characters.build(id, colors)` — `colors` is the character's `colors` block `{primary, secondary, kart, skin}`). The driver models are **chibi-proportioned** (oversized head) for recognizability; per-character builders give each its signature silhouette: Mario/Luigi/Wario share `humanoidBody` (cap+emblem, big round nose, mustache — Wario's is the pointed variant), **Peach** has a flared gown + blonde hair + crown, **Toad** a big spotted mushroom cap, **Yoshi** a big snout + red saddle + orange boots, **DK** a broad furred torso + red "DK" tie, **Bowser** a spiked shell + horns + red mane + fangs. Helpers: `cap`/`mustache`/`emblem`/`eye`/`dome` (avoid `CapsuleGeometry` — not in r128). The kart chassis is in `kart.js` (`MK.buildChassis(color, accent)`, `accent`=`colors.secondary`): a friendly **go-kart** (rounded tub + cowl, fat wheels with accent hubcaps, twin upright chrome exhausts, number roundel). **Contract: `chassis.userData.wheels` (4, spun on `rotation.x`) and `chassis.userData.steerPivots` (front 2, steered on `rotation.y`) must exist** — `kart.js` drives them every frame. All user-facing text is **English**; the `jp` field on characters/courses is now unused metadata.
